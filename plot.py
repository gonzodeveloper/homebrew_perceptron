...
class Plotter:
    ...
    def generate_points(self, n, dim):
        '''
        Create n random points in a number of arbitrary dimensions
        :param n: number of random points
        :param dim: dimensionality of points desired
        :return: numpy array of random points (i.e. feature vectors
        '''
        return np.random.uniform(low=-1, high=1, size=(n, dim))


    def generate_labels_linear(self, features):
        '''
        Create an array of linearly separable labels for a list of feature vectors
        ...
        norm = np.random.uniform(low=-1, high =1, size=len(features[0]))
    ...
    def generate_labels_xor(self, features):
        '''
    ...
    def plot_graph(self, model, features, labels, epochs, dir):
        '''
...
if __name__ == "__main__":
    ...
    plot = Plotter()
    features = plot.generate_points(50,3)
    lab_lin = plot.generate_labels_linear(features)
    lab_xor = plot.generate_labels_xor(features)

    lin_model = Perceptron(inputs=3)
    xor_model = Perceptron(inputs=3)

    plot.plot_graph(lin_model, features, lab_lin, epochs=20, dir="img/lin_img1")
    plot.plot_graph(xor_model, features, lab_xor, epochs=20, dir="img/xor_img1")
